...
2. **index / .crate 缓存**在宿主机 `/opt/data/freighter`，Job 里挂载为 `/freighter`；**不要**在集群里重新 clone 整份 index。
...
```text
progress: [##----------------------------]   0.8%  16000/1850000  eta=48.2h
...
- `[OK] xxx pushed` 会刷日志，进度条钉在底部（sticky）。
