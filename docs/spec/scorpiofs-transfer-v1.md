...
论文研究问题、相关工作、候选选择算法、正确性不变量和公平实验要求见 [研究设计](scorpiofs-transfer-research.md)。本文定义运输契约；其中 75% 缺失阈值、1 ms 聚合窗口和包大小是静态基线参数，不作为新颖性或普遍性能保证。
...
ScorpioFS 的静态基线选择规则（另与研究设计中的成本选择器对照）：
...
| 工作负载 | 核心指标 |
| --- | --- |
| 1,000 × 8 KiB 同 source/目录文件，冷缓存 | 需求已知且按 128 个聚合时为 8 次包请求，目录分页另计；纯按需另报实际数量，对比同并发单对象 GET |
